 ...
 * ```
 */
class PokerSeat extends BaseEventEmitter {
    /**************************************************************************************************************
    ...
    __checkSeatVacancy(event, next) {
        if (this.isOccupied()) {
            logger.log(LogLevel.WARN, "Failed to occupy seat: seat is already occupied.", {
                seatId: this.getId(),
            });
            return;
        }
    ...
    __checkSeatOccupancy(event, next) {
        event.lastModifiedAt = new Date();
        if (!this.isOccupied()) {
            logger.log(LogLevel.WARN, "Failed to vacate seat: seat is already vacant.", {
                seatId: this.getId(),
            });
            return;
        }
