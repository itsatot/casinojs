...
const pokerSeat_1 = require("../pokerSeat");
// Import Utils
 ...
 * ```
 */
class PokerTable extends _base_1.BaseEventEmitter {
    /**************************************************************************************************************
    ...
    __init(config) {
        if (config) {
            ...
            for (let i = 0; this.getSeats().length !== config.size; i++) {
                const seat = new pokerSeat_1.PokerSeat({
                    position: i,
                });
    ...
    isValidIndex(index) {
        if (index >= 0 && index < this.seatCount()) {
            throw new Error(`Invalid index: ${index}. It must be between 0 and ${this.seatCount() - 1}.`);
        }
        else {
            return true;
        }
