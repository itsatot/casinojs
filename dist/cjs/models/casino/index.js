...
// Import Enums
const casinoEventName_1 = require("../../enums/casinoEventName");
// Import Models
...
class Casino extends events_1.EventEmitter {
    ...
    isValidIndex(index) {
        if (index >= 0 && index < this.roomCount()) {
            throw new Error(`Invalid index: ${index}. It must be between 0 and ${this.roomCount() - 1}.`);
        }
        else {
            return true;
        }
    ...
    _setRooms(rooms) {
        this.__rooms = rooms;
        this.emit(casinoEventName_1.CasinoEventName.ROOMS_SET, this.getRooms());
    ...
    _createRoom(config) {
        ...
        this.emit(casinoEventName_1.CasinoEventName.ROOM_CREATED, room);
        return room;
    }
    /**
     * #### Description
    ...
    _addRoom(room) {
        this.__rooms.push(room);
        this.emit(casinoEventName_1.CasinoEventName.ROOM_ADDED, this.getRooms());
        return this.getRooms();
    }
    /**
     * #### Description
    ...
    _deleteRoom(index) {
        if (this.isValidIndex(index)) {
            ...
            this.emit(casinoEventName_1.CasinoEventName.ROOM_DELETED, this.getRooms());
        }
        return this.getRooms();
    }
}
