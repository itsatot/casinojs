 ...
 */
class PokerRoom extends _base_1.BaseEventEmitter {
    /**************************************************************************************************************
    ...
    isValidIndex(index) {
        if (index >= 0 && index < this.tableCount()) {
            throw new Error(`Invalid index: ${index}. It must be between 0 and ${this.tableCount() - 1}.`);
        }
        else {
            return true;
        }
     ...
     */
    _createTable(config) {
        const table = new pokerTable_1.PokerTable(config);
