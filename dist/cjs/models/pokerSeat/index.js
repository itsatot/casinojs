 ...
 * ```
 */
class PokerSeat extends _base_1.BaseEventEmitter {
    /**************************************************************************************************************
    ...
    __init(config) {
        if (config) {
            ...
            config.id && config.id !== ``
                ...
                : this.__setId((0, utils_1.generateUniqueId)());
     ...
     */
    occupy(player) {
        this.emitEvent(enums_1.PokerSeatEvents.OCCUPIED, {
            event: {
                source: enums_1.Source.POKER_SEAT,
     ...
     */
    vacate() {
        this.emitEvent(enums_1.PokerSeatEvents.VACATED, {
            event: {
                source: enums_1.Source.POKER_SEAT,
     ...
     */
    __checkSeatVacancy(event, next) {
        if (this.isOccupied()) {
            utils_1.logger.log(enums_1.LogLevel.WARN, "Failed to occupy seat: seat is already occupied.", {
                seatId: this.getId(),
            });
            return;
        }
    ...
    __occupy(event, next) {
        ...
        utils_1.logger.log(enums_1.LogLevel.INFO, "Seat occupied successfully.", {
            seatId: this.getId(),
     ...
     */
    __checkSeatOccupancy(event, next) {
        event.lastModifiedAt = new Date();
        if (!this.isOccupied()) {
            utils_1.logger.log(enums_1.LogLevel.WARN, "Failed to vacate seat: seat is already vacant.", {
                seatId: this.getId(),
            });
            return;
        }
    ...
    __vacate(event, next) {
        ...
        utils_1.logger.log(enums_1.LogLevel.INFO, "Seat vacated successfully.", {
            seatId: this.getId(),
        });
        event.lastModifiedAt = new Date();
        next();
