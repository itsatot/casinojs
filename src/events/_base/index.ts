 ...
 *
 * #### Purpose
 * The `BaseEvent` interface establishes a standardized structure for all events across the library.
 * It is designed to be extended by specific event types, providing core fields while supporting extensibility
 * for additional metadata and payload properties. The `BaseEvent` structure ensures that all events are
 ...
 *   it possible to adapt the structure for a range of scenarios. The `BaseEvent` interface itself allows
 *   sibling properties to be added as needed by extending interfaces.
 *
 * #### Usage
 * `BaseEvent` provides the foundation for defining any event within the library. It is intended to be
 * extended by event-specific interfaces that may introduce additional fields within `data`. In this way,
 * `BaseEvent` serves as a flexible base for creating a wide variety of standardized events across the
 * library’s features.
 *
 * @example
 * ```typescript
...
interface BaseEvent<T = any> {
   ...
   * Allows additional sibling properties within `BaseEvent` for extensibility.
   *
   * #### Usage
   * Supports additional fields at the root level of `BaseEvent`, providing flexibility
